...
trap 'exec $HOME/.panel/ftp/user_ftps_category.sh' SIGINT
...
do
  ...
  then
    ...
    then
      ...
      sleep 5
      exec $HOME/.panel/ftp/user_ftps_category.sh
    else
...
sudo /usr/local/panel/bin/add_new_ftp_user.sh $entered_name_new_ftp_user $full_path_new_ftp_account 
exec $HOME/.panel/ftp/user_ftps_category.sh

...
trap 'exec $HOME/.panel/www/root_wwws_category.sh' SIGINT
...
do
  ...
  then
    ...
    then
      ...
      sleep 5
      exec $HOME/.panel/www/root_wwws_category.sh
    else
...
while true
do
  read -p "Enter owner new site: " entered_owner_new_site
  #Turn owner into lower case
  entered_owner_new_site=$(echo $entered_owner_new_site | tr '[:upper:]' '[:lower:]')

  #Check symbols in owner
  if echo $entered_owner_new_site | grep -q "[^a-z0-9_.-]"
  then
    if [ $wrong_count = "2" ]
    then
      echo 'ERROR! You are mistaken 3 times. Return to the previous menu after 5 seconds...'
...
sudo /usr/local/panel/bin/add_new_site.sh $entered_name_new_site $entered_phpversion_new_site $ssl_new_site $backup_new_site $entered_owner_new_site
exec $HOME/.panel/www/root_wwws_category.sh

...
while true
do
  read -p "Enter owner DB: " entered_owner_new_db
  #Turn owner into lower case
  entered_owner_new_db=$(echo $entered_owner_new_db | tr '[:upper:]' '[:lower:]')  

  #Check symbols in owner
  if echo $entered_owner_new_db | grep -q "[^a-z0-9_.-]"
  then
    if [ $wrong_count = "2" ]
    then
      echo 'ERROR! You are mistaken 3 times. Return to the previous menu after 5 seconds...'
...
sudo /usr/local/panel/bin/add_new_db.sh $entered_name_new_db $entered_owner_new_db
exec $HOME/.panel/db/root_dbs_category.sh

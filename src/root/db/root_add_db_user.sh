...
trap 'exec $HOME/.panel/db/root_dbs_category.sh' SIGINT
...
while true
do
  read -p "Enter owner new DB user: " entered_owner_new_db_user
  #Turn owner into lower case
  entered_owner_new_db_user=$(echo $entered_owner_new_db_user | tr '[:upper:]' '[:lower:]')

  #Check symbols in owner
  if echo $entered_owner_new_db_user | grep -q "[^a-z0-9_.-]"
  then
    if [ $wrong_count = "2" ]
    then
      echo 'ERROR! You are mistaken 3 times. Return to the previous menu after 5 seconds...'
      ...
      echo 'You enter owner DB name with wrong characters. Allowed characters is a-Z , 0-9 and symbol _'
      wrong_count=$[ $wrong_count + 1 ]
      continue
    fi
  fi
...
sudo /usr/local/panel/bin/add_new_db_user.sh $entered_name_new_db_user $entered_owner_new_db_user
exec $HOME/.panel/db/root_dbs_category.sh

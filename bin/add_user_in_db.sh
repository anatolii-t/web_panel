...
action_perfomed="add_user_in_db"
start_message="Start adding user ${neeaded_user} in DB ${neeaded_db}"
...
then
  message="ERROR! User '$neeaded_user' already added in DB '$neeaded_db'! Exiting now!"
  echo "\\t$message" | logging show
  sleep 5
  exit 1
fi
...
#check_count_users
count_users=`mysql_connector "SELECT users FROM data_bases WHERE name='$neeaded_db'" | wc -w`
if [ $user_in_system_is_exist -gt 5 ]
then
  message="ERROR! In DB '$neeaded_db' already added 5 users (maximum)! Exiting now!"
  echo "\\t$message" | logging show
  sleep 5
  exit 1
fi
...
mysql_connector "UPDATE data_bases SET users=CONCAT_WS(', ',users,'$neeaded_user') WHERE name='$neeaded_db';"

mysql -e "grant all privileges on ${neeaded_db}.* to '${neeaded_user}'@'localhost';"
